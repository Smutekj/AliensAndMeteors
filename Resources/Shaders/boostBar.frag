...
{   
    ...
    vec2 uv = vec2(v_tex_coord.x, v_tex_coord.y) - vec2(0.5, 0.5);
    float sdf = sdBoxRC(vec2(uv.x, uv.y), vec2(0.5, 0.5), vec2(0.2, 0.2*2.), 0.2);
    float rect_border = smoothstep(-0.1, -0.05, sdf);
    vec2 size = vec2(0.5, 0.5);
    vec2 corner_radius = vec2(0.2 / u_bar_aspect_ratio, 0.2);
    // FragColor = vec4(1.0,1.,1.,1.);
    // if(uv.x > size.x - corner_radius.x && uv.y > size.y - corner_radius.y)
    // {
    //     sdf = distance(uv, vec2(size.x - corner_radius.x, size.y - corner_radius.y));
    ...
    // FragColor = vec4(v_tex_coord, 0., 1.);
    return;
